...
pub(crate) fn enable() {
    crc().ctrl().modify(|_, w| w.set_en(true));
}

pub(crate) fn disable() {
    crc().ctrl().modify(|_, w| w.set_en(false));
}

pub(crate) fn reset() {
    crc().cmd().write_value(Default::default());
    crc().init().write_value(Default::default());
...
pub(crate) fn set_algo_16(algo: &Algorithm<u16>) {
    ...
    crc()
        .poly()
        .write(|w| unsafe { w.set_poly(algo.poly.reverse_bits()) });
    crc()
        .init()
        .write(|w| unsafe { w.set_init(algo.init.reverse_bits() as u32) });
}
...
pub(crate) fn set_algo_32(algo: &Algorithm<u32>) {
    ...
    crc()
        .init()
        .write(|w| unsafe { w.set_init(algo.init.reverse_bits()) });
}

pub(crate) fn auto_init_set() {
    crc().ctrl().modify(|_, w| w.set_autoinit(true));
}
...
pub(crate) fn data_u16(rev_bits: bool) -> u16 {
    match rev_bits {
        true => (crc().datarev().read().datarev() & 0x0000FFFF) as u16,
        false => (crc().data().read().data() & 0x0000FFFF) as u16,
    }
}

pub(crate) fn data_u32(rev_bits: bool) -> u32 {
    match rev_bits {
        true => crc().datarev().read().datarev(),
        false => crc().data().read().data(),
    }
}
